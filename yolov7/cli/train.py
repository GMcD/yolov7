...
def train(hyp, opt, device, tb_writer=None):
    ...
    if rank in [-1, 0]:
        ...
        run_id = torch.load(weights, map_location=device).get('wandb_id') if weights.exists() else None
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(300):  # generations to evolve
            if Path('evolve.txt').exists():  # if evolve.txt exists: select best hyps and mutate
                ...
                ng = len(meta)
                v_ones = np.ones(ng)
                while all(v_ones == 1):  # mutate until a change occurs (prevent duplicates)
                    v_ones = (g * (npr.random(ng) < mp) * npr.randn(ng) * npr.random() * s + 1).clip(0.3, 3.0)
                for i, k in enumerate(hyp.keys()):  # plt.hist(v.ravel(), 300)
                    hyp[k] = float(x[i + 7] * v_ones[i])  # mutate
